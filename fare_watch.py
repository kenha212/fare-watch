...
def search_cheapest_fare(cfg):
    """Returns (price, airline, price_verdict) for the cheapest itinerary,
    or (None, None, None) if nothing came back."""
    body = {
    ...
    cheapest = min(offers, key=lambda o: o["total_price_as_number"])
    airline = cheapest.get("departure_flight_airline")
    verdict = cheapest.get("price_range_in_relation_to_other_periods")
    return cheapest["total_price_as_number"], airline, verdict


def compute_signal(history, target, depart_date, verdict=None):
    if not history:
    ...
    if target and latest <= target:
        return "buy", f"Latest fare ${latest:.0f} is at or under your target ${target:.0f}."
    ...
    if len(history) >= 3 and latest <= lowest * 1.03 and 0 <= days_to_departure <= 45:
        return (
            "buy",
            f"Latest fare ${latest:.0f} is near the lowest logged (${lowest:.0f}), "
    ...
    if prev is not None and latest > prev and 0 <= days_to_departure <= 60:
        return "rising", f"Fare rose from ${prev:.0f} to ${latest:.0f} — worth deciding soon."
...
def main():
    ...
    price, airline, verdict = search_cheapest_fare(cfg)
    ...
    history.append({"date": today, "price": price, "airline": airline, "verdict": verdict})
    save_json(HISTORY_PATH, history)

    signal, message = compute_signal(history, cfg.get("target_price_aud"), cfg["depart_date"], verdict)
    print(f"[{today}] {cfg['origin']}->{cfg['destination']}: ${price:.0f} ({signal}) - {message}")

    if signal in ("buy", "rising"):
        subject = f"[Fare Watch] {cfg['origin']}-{cfg['destination']}: {signal.upper()} — ${price:.0f} AUD"
        body = f"{message}\n\nLatest fare: ${price:.0f} AUD" + (f" on {airline}" if airline else "") + "."
